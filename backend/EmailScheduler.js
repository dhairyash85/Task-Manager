...
cron.schedule("* * * * *", async () => {
    const now = new Date();
    const oneHourLater = new Date(now.getTime() - 60 * 60 * 1000); // Subtract one hour to get the time when the task is due
    console.log("Time to delete tasks:", oneHourLater);
  
    // Find tasks that are due now
    const tasksDueNow = await Tasks.find({
      date: new Date(oneHourLater.toISOString().split("T")[0]),
      time: oneHourLater.toISOString().split("T")[1].slice(0, 5),
    });
